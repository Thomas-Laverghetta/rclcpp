...
TEST_F(TestBasicServer, publish_result_rcl_errors) {
  // This exception leaks out of EXPECT_THROW somehow...
  // {
  //   auto mock = mocking_utils::patch_and_return(
  //     "lib:rclcpp_action", rcl_action_server_goal_exists, false);
  //   EXPECT_THROW(SetupActionServerAndSpin(RequestType::GOAL), std::runtime_error);
  // }
  {
    auto mock = mocking_utils::patch_and_return(
...
TEST_F(TestBasicServer, publish_feedback_rcl_errors) {
  {
    auto mock = mocking_utils::patch_and_return(
      "lib:rclcpp_action", rcl_action_publish_feedback, RCL_RET_ERROR);
    SetupActionServerAndSpin(RequestType::GOAL);
    EXPECT_THROW(rclcpp::spin_some(node_), rclcpp::exceptions::RCLError);
  }
}
...
TEST_F(TestBasicServer, execute_goal_request_received_take_failed) {
  {
    ...
    try {
      ...
      ADD_FAILURE() << "SetupActionServerAndSpin did not threw, but was expected to";
...
TEST_F(TestBasicServer, execute_result_request_received_take_failed)
{
  ...
  auto mock = mocking_utils::patch_and_return(
    "lib:rclcpp_action", rcl_action_take_result_request, RCL_RET_ACTION_SERVER_TAKE_FAILED);
  auto future = result_client->async_send_request(request);
  auto result_code =
    rclcpp::spin_until_future_complete(node_, future, std::chrono::milliseconds(100));
  EXPECT_EQ(rclcpp::FutureReturnCode::TIMEOUT, result_code);
}
